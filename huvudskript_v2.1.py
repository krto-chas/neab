...
from utils.docx_template import create_neab_report as create_neab_docx_report
...
class NEABSecurityAssessment:
    ...
    def kor_komplett_analys(self):
        ...
        progress = ProgressBar(5, "Skapar filer")
        ...
        try:
            # DOCX - använd samma data som PDF för konsistens
            docx_path = self.output_dir / f"NEAB_Ledningsrapport_{self.timestamp}.docx"

            # Använd samma pdf_data som ovan
            if 'pdf_data' in locals():
                create_neab_docx_report(docx_path, pdf_data)
                ConsoleFormatter.progress_complete("DOCX-rapport skapad", docx_path)
            else:
                # Fallback om pdf_data inte finns
                docx_data = self._prepare_pdf_data(resultat)
                create_neab_docx_report(docx_path, docx_data)
                ConsoleFormatter.progress_complete("DOCX-rapport skapad", docx_path)

        except Exception as e:
            ConsoleFormatter.warning(f"DOCX-generering misslyckades: {e}")
            logger.exception("DOCX-fel")
