...
class NEABDocxTemplate:
    ...
    def add_executive_summary(self, data):
        ...
        for i, action in enumerate(data.get('priority_actions', [])[:5], 1):
            p = self.doc.add_paragraph(f"{i}. {action}")
            p.style = 'NEAB Body'
    ...
    def add_risk_summary(self, data):
        ...
        for i, risk in enumerate(data.get('top_risks', [])[:3], 1):
            p = self.doc.add_paragraph(f"{i}. {risk}")
            p.style = 'NEAB Body'
            p.paragraph_format.left_indent = Pt(18)
            p.paragraph_format.first_line_indent = Pt(-18)
            p.paragraph_format.space_after = Pt(4)
        ...
        if data.get('heatmap_path'):
            heatmap_path = Path(data.get('heatmap_path'))
            if heatmap_path.exists():
                try:
                    self.doc.add_paragraph()
                    p = self.doc.add_paragraph('Riskmatris - Visuell översikt:')
                    p.style = 'NEAB Heading 2'
                    
                    # Lägg till bilden
                    self.doc.add_picture(str(heatmap_path), width=Inches(6))
                    ...
                    logger.warning(f"Kunde inte lägga till heat map i DOCX: {e}")
                    # Fallback till text
                ...
                p = self.doc.add_paragraph('[Heat map kunde inte hittas]')
                run = p.runs[0]
                run.font.italic = True
                run.font.color.rgb = NEABColors.NEUTRAL
    ...
    def add_mitigation_summary(self, data):
        ...
        if priority_actions and priority_actions[0] != 'Okänd åtgärd':
            ...
            for i, action in enumerate(priority_actions[:5], 1):
                ...
                p = self.doc.add_paragraph(f"{i}. {action_text}")
                p.style = 'NEAB Body'
                p.paragraph_format.left_indent = Pt(18)
                p.paragraph_format.first_line_indent = Pt(-18)
                p.paragraph_format.space_after = Pt(4)
